<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="font-bold text-2xl text-gray-900">Add Teacher</h2>
                <p class="text-gray-600 text-sm mt-1">Create a new teacher profile and assign classes</p>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-lg shadow-lg border border-gray-100">
                ...
                <div class="border-b border-gray-200 px-6 py-6 bg-gradient-to-r from-blue-50 to-indigo-50">
                    ...
                    <p class="text-sm text-gray-600 mt-1">Please fill in all the required fields below</p>
                </div>

                <!-- Form Body -->
                <form method="POST" action="{{ route('teachers.store') }}" class="p-8">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 gap-8 sm:grid-cols-2 mb-8">
                        ...
                        <!-- Payment Start Month -->
                        <div>
                            <x-input-label for="payment_month" :value="__('Payment Start Month')" class="font-semibold mb-2" />
                            <x-text-input 
                                id="payment_month" 
                                name="payment_month" 
                                type="month" 
                                class="mt-1 block w-full border-gray-300 focus:border-blue-500 focus:ring-blue-500 rounded-lg shadow-sm" 
                                :value="old('payment_month')" 
                            />
                            <p class="text-gray-500 text-xs mt-1">Select when to start salary payments</p>
                        </div>

                        <!-- Payment Start Date (Auto-calculated but editable) -->
                        <div>
                            ...
                            />
                            <p class="text-blue-600 text-xs mt-1 font-semibold" id="payment_date_info">Payment date will be set to end of selected month</p>
                    ...
                    <div class="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-8">
                        <div class="mb-4">
                            <x-input-label for="assigned_classes" :value="__('Assign Classes (Optional)')" class="font-semibold mb-3 block" />
                            ...
                            @else
                                <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                    @foreach($classrooms as $classroom)
                                        @php
                                            $assignedClasses = old('assigned_classes') ? explode(',', old('assigned_classes')) : [];
                                            $isChecked = in_array($classroom->id, $assignedClasses);
                                        @endphp
                                        <label class="flex items-center p-3 border-2 border-blue-200 rounded-lg hover:border-blue-400 hover:bg-blue-100 cursor-pointer transition">
                                            <input 
                                                type="checkbox" 
                        ...
                        <input type="hidden" id="assigned_classes" name="assigned_classes_hidden">
                    </div>
                    ...
                    <div class="bg-green-50 border border-green-200 rounded-lg p-6 mb-8">
                        ...
                        <div id="salary-components-container" class="space-y-3 mb-4">
                            <!-- Default Basic Salary Component -->
                        ...
                        <div class="bg-white border-2 border-green-500 rounded-lg p-4">
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-2xl font-bold text-green-600" id="total-salary-display">Rs 0.00</span>
                            </div>
                        </div>
                    ...
                    <div class="bg-purple-50 border border-purple-200 rounded-lg p-6 mb-8">
                        <div class="flex items-center">
                            ...
                            <label for="active" class="ml-3 font-medium text-gray-800 cursor-pointer">
                                Teacher Status: <span id="status-label" class="text-green-600 font-bold">Active</span>
                            </label>
                        </div>
                    ...
                    <div class="flex items-center gap-4 pt-6 border-t border-gray-200">
                        ...
                        <a href="{{ route('teachers.index') }}" class="px-4 py-2 text-gray-700 bg-gray-200 hover:bg-gray-300 rounded-lg font-medium transition">
                            Cancel
                        </a>
                    </div>
                </form>
    ...
    <script>
        ...
        // Payment date calculation and handling
        const paymentMonthInput = document.getElementById('payment_month');
        const paymentStartDateInput = document.getElementById('payment_start_date');
        const paymentDateInfo = document.getElementById('payment_date_info');

        function calculatePaymentDate() {
            const selectedMonth = paymentMonthInput.value; // Format: YYYY-MM
            ...
            if (selectedMonth) {
                ...
                paymentDateInfo.textContent = `Payment will start from ${paymentDate.getDate()}${getDaySuffix(paymentDate.getDate())} of ${monthName}`;
                paymentDateInfo.classList.remove('text-gray-500');
                paymentDateInfo.classList.add('text-blue-600', 'font-semibold');
            } else {
                paymentStartDateInput.value = '';
                paymentDateInfo.textContent = 'Payment date will be set to end of selected month';
                paymentDateInfo.classList.remove('text-blue-600', 'font-semibold');
                paymentDateInfo.classList.add('text-gray-500');
            }
        }

        function updatePaymentDateInfo() {
            const selectedDate = paymentStartDateInput.value;
            
            if (selectedDate) {
                const date = new Date(selectedDate + 'T00:00:00');
                const monthName = date.toLocaleString('default', { month: 'long', year: 'numeric' });
                paymentDateInfo.textContent = `Payment will start from ${date.getDate()}${getDaySuffix(date.getDate())} of ${monthName}`;
                paymentDateInfo.classList.remove('text-gray-500');
                paymentDateInfo.classList.add('text-blue-600', 'font-semibold');
            }
        }
        ...
        if (paymentMonthInput) {
            paymentMonthInput.addEventListener('change', calculatePaymentDate);
        }

        if (paymentStartDateInput) {
            paymentStartDateInput.addEventListener('change', updatePaymentDateInfo);
        }
        ...
        // Initialize on page load
        document.addEventListener('DOMContentLoaded', function() {
            // If there's already a payment month selected, calculate the date
            if (paymentMonthInput.value) {
                calculatePaymentDate();
            }
            // If there's a payment date, show the info
            if (paymentStartDateInput.value) {
                updatePaymentDateInfo();
            }
            // Calculate initial salary total
