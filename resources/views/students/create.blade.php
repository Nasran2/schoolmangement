<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="font-bold text-2xl text-gray-900">Add Student</h2>
                <p class="text-gray-600 text-sm mt-1">Create a new student profile and enroll them in a class</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-12 bg-gray-50 min-h-screen">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-lg shadow-lg border border-gray-100">
                ...
                <!-- Form Body -->
                <form method="POST" action="{{ route('students.store') }}" class="p-8" id="student-create-form">
                    @csrf
                    ...
                    <input type="hidden" id="full_name_hidden" name="name" value="{{ old('name') }}" />

                    <!-- Combined Admission & Personal UI -->
                    @include('students._admission_fields')
                        ...
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            <div>
                                ...
                                <select 
                                    ...
                                    required
                                >
                                    <option value="" disabled {{ old('class_room_id') ? '' : 'selected' }}>Select class</option>
                                    @foreach ($classRooms as $cr)
                                        <option value="{{ $cr->id }}" data-monthly-fee="{{ $cr->monthly_fee }}" @selected((string) old('class_room_id') === (string) $cr->id)>
                            ...
                            <div>
                                <x-input-label for="fee_start_date" :value="__('Payment Start Date')" class="font-semibold mb-2" />
                                <x-text-input 
                                    id="fee_start_date" 
                                    name="fee_start_date" 
                                    type="text" 
                                    ...
                                    class="mt-1 block w-full border-gray-300 focus:border-green-500 focus:ring-green-500 rounded-lg shadow-sm" 
                                    :value="old('fee_start_date', \Carbon\Carbon::now()->format('10-m-Y'))" 
                                />
                                <x-input-error class="mt-2" :messages="$errors->get('fee_start_date')" />
                                <p class="mt-1 text-xs text-gray-500">First month when monthly fee is due.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-amber-50 border border-amber-200 rounded-lg p-6 mb-8">
                        ...
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            <div>
                                ...
                                <p class="text-gray-500 text-xs mt-1">Auto-filled from selected class</p>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="mt-1 flex items-center gap-2 px-4 py-3 border-2 border-amber-300 rounded-lg bg-amber-100 text-amber-900 font-semibold">
                                    ...
                                    <span id="due_amount_display" class="text-xl">0.00</span>
                    ...
                    <div class="bg-green-50 border border-green-200 rounded-lg p-6 mb-8">
                        ...
                        <div class="flex items-center">
                            ...
                            <label for="active" class="ml-3 font-medium text-gray-800 cursor-pointer">
                                Student Status: <span id="status-label" class="text-green-600 font-bold">Active</span>
                            </label>
                        </div>
                    ...
                    <div class="flex items-center gap-4 pt-6 border-t border-gray-200">
                        ...
                        <a href="{{ route('students.index') }}" class="px-4 py-2 text-gray-700 bg-gray-200 hover:bg-gray-300 rounded-lg font-medium transition">
                            Cancel
                        </a>
                    </div>
                </form>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Default Payment Start Date to the 10th of the current month
            if (feeStartInput && !feeStartInput.value) {
                const now = new Date();
                const d = new Date(now.getFullYear(), now.getMonth(), 10);
                const yyyy = String(d.getFullYear());
                const mm = String(d.getMonth() + 1).padStart(2, '0');
                const dd = String(d.getDate()).padStart(2, '0');
                feeStartInput.value = `${dd}-${mm}-${yyyy}`;
            }
