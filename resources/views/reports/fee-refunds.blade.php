<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="font-bold text-2xl text-gray-900">Refund / Cancellation Report</h2>
                <p class="text-gray-600 text-sm mt-1">All refund adjustments with filters and exports.</p>
            </div>
            <div class="flex gap-2">
                ...
                <a class="inline-flex items-center px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg font-semibold" href="{{ request()->fullUrlWithQuery(['pdf' => 1]) }}">Download PDF</a>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white rounded-lg shadow border border-gray-100 p-6">
                <div class="flex items-center justify-between mb-4">
                    <div class="text-sm text-gray-600">Total Refunds: <span class="font-semibold text-gray-900">Rs {{ number_format((float) $totalAmount, 2) }}</span></div>
                </div>
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white">
                            @forelse($items as $a)
                                <tr>
                                    ...
                                    <td class="px-4 py-3 text-sm text-right font-semibold text-rose-700">{{ number_format((float) $a->amount, 2) }}</td>
                                    <td class="px-4 py-3 text-sm text-gray-700">{{ $a->reason ?: '-' }}</td>
                                    <td class="px-4 py-3 text-sm text-gray-700">{{ $a->creator?->name ?: '-' }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td class="px-4 py-6 text-center text-sm text-gray-600" colspan="8">No refunds found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
