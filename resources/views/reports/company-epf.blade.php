<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="font-bold text-2xl text-gray-900">Company EPF Report</h2>
                <p class="text-gray-600 text-sm mt-1">Company EPF contribution from teacher salary payments</p>
            </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-lg shadow-lg border border-gray-100 mb-8">
                <div class="border-b border-gray-200 px-6 py-6 bg-gradient-to-r from-blue-50 to-indigo-50">
                    <h3 class="text-lg font-semibold text-gray-800">Filter Company EPF</h3>
                        ...
                        <div class="flex items-end gap-2 lg:col-span-2">
                            ...
                            @can('reports.download')
                                <a class="flex-1 inline-flex items-center justify-center px-4 py-2.5 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-lg transition shadow-sm"
                                   href="{{ route($routeName ?? 'reports.company_epf', array_merge(request()->query(), ['pdf' => 1])) }}">PDF</a>
                                <a class="flex-1 inline-flex items-center justify-center px-4 py-2.5 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-lg transition shadow-sm"
                                   href="{{ route($routeName ?? 'reports.company_epf', array_merge(request()->query(), ['download' => 1])) }}">CSV</a>
                            @endcan
                        </div>
                    </form>
                </div>
            ...
            <div class="bg-white rounded-lg shadow-lg border border-gray-100 overflow-hidden mb-8">
                <div class="px-6 py-4 border-b border-gray-200">
                    <h3 class="text-lg font-semibold text-gray-800">Company EPF Summary</h3>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">Total EPF</th>
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 text-sm text-right">
                                        <a class="inline-flex items-center px-3 py-1.5 bg-gray-200 hover:bg-gray-300 text-gray-800 rounded-md"
                                           href="{{ route($routeName ?? 'reports.company_epf', array_merge(request()->query(), ['teacher_id' => $row['teacher']?->id])) }}">
                                            View
                                        </a>
                                    </td>
                                </tr>
            ...
            @if(($groupByMonth ?? false) && !empty($monthTotals))
                <div class="bg-white rounded-lg shadow-lg border border-gray-100 overflow-hidden mb-8">
                    <div class="px-6 py-4 border-b border-gray-200">
                        <h3 class="text-lg font-semibold text-gray-800">EPF Month Totals</h3>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div class="bg-white rounded-lg shadow-lg border border-gray-100 p-6">
                    <p class="text-sm text-gray-600 font-medium">Total EPF</p>
                    <p class="text-3xl font-bold text-indigo-600 mt-2">Rs {{ number_format($totalAmount, 2) }}</p>
                </div>
                <div class="bg-white rounded-lg shadow-lg border border-gray-100 p-6">
                    <p class="text-sm text-gray-600 font-medium">Average EPF</p>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">EPF</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-200">
