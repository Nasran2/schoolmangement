<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h1 class="text-xl font-bold text-gray-800">Seminars Collection</h1>
            ...
            <div class="px-6 py-4 border-b border-gray-100">
                <form method="GET" class="grid grid-cols-1 md:grid-cols-6 gap-3">
                    ...
                    <div>
                        ...
                        <input type="text" name="q" value="{{ $filters['q'] ?? '' }}" placeholder="Seminar name" class="rounded-md border-gray-300 w-full" />
                    </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">Teacher Paid</th>
                            <th class="px-6 py-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">Teacher Due</th>
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white">
                        @foreach($rows as $r)
                            <tr>
                                <td class="px-6 py-4">{{ $r->seminar_name }}</td>
                                <td class="px-6 py-4">{{ \Carbon\Carbon::parse($r->date)->format('Y-m-d') }}</td>
                                ...
                                <td class="px-6 py-4 text-right font-mono">{{ number_format((float)$r->teacher_payment, 2) }}</td>
                                <td class="px-6 py-4 text-right font-mono">{{ number_format((float)($r->teacher_paid ?? 0), 2) }}</td>
                                <td class="px-6 py-4 text-right font-mono">{{ number_format((float)($r->teacher_due ?? $r->teacher_payment), 2) }}</td>
